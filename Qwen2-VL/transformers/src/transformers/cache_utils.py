...
class DynamicCache(Cache):
    ...
    def filter_indices(self, keep_indexs: torch.Tensor): # HACK:edited by zichen
        ...
        Args:
            keep_indexs (`torch.Tensor`): Indices to keep in the sequence dimension.
        """
        for layer_idx in range(len(self.key_cache)):
            self.key_cache[layer_idx] = self.key_cache[layer_idx][:, :, keep_indexs, :]
            self.value_cache[layer_idx] = self.value_cache[layer_idx][:, :, keep_indexs, :]
        self._seen_tokens = len(keep_indexs)

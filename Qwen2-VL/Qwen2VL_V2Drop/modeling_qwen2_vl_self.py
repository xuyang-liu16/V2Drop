            ...
            if self.gradient_checkpointing and self.training:
                ...
                if DART_config is not None :
                    ...
                    if decoder_layer.self_attn.layer_idx == 0:
                        self.image_token_amount_after_selection = image_token_length
                    target_layer_indices = {3, 16, 22}
                    if decoder_layer.self_attn.layer_idx in target_layer_indices and current_hidden_states.size(1) > 1 and self.image_token_amount_after_selection > 0:
                        ...
                        candidate_indices = torch.arange(image_token_start_index, self.image_token_amount_after_selection + image_token_start_index,
                                                         device=device)
                        ...
                        retained_image_tokens_index = candidate_indices[topk_indices]
                        current_seq_length = current_hidden_states.size(1)

                        keep_indexs = torch.cat((torch.arange(image_token_start_index,device=device), retained_image_tokens_index, torch.arange(image_token_start_index+self.image_token_amount_after_selection,current_seq_length,device=device)))
                        ...
                        self.image_token_amount_after_selection = retained_image_tokens_index.shape[0]
...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        # cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)  # ! debug
...
class Qwen2VLFlashAttention2(Qwen2VLAttention):
    ...
    def forward(
        ...
        # ) # ! debug
        rotary_seq_len = (
            position_ids[:, -1].max().item() + 1 if position_ids is not None else kv_seq_len
        )
